...
no_noise = 'Users/ah13558/Documents/PhD/Image audio similarity/nonoise.wav'
...
for fold in tqdm(folders):
    ...
    chrom = sio.loadmat(fold + "Noise/chrom.mat")['chroma']
    chromas = []
    names = []
    for i in range(0, chrom[0].shape[0]):
        try:
            names.append(float(chrom[0][i][0][0].replace('-', '.').replace('.wav', '')))
            chromas.append(chrom[0][i][1][0][0])
        except ValueError:
            y = chrom[0][i][1][0][0]
    ...
    for f in (glob.glob(fold + "Noise/*.wav")):
        bitrates.append(float(f.split('/')[-1].replace('.wav', '').replace('-', '.')))
    ...
    chrom_lap.append(chrom_lap_temp)
    chrom_eucl.append(chrom_eucl_temp)
